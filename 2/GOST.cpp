...
bool modGOST::encrypt()
{
    ...
    pbkdf.DeriveKey(key.data(), key.size(), 0, (byte*)psw.data(), psw.size(), (byte*)world.data(), world.size(), 1024, 0.0f);
    cout << "Key: ";
    ...
    ofstream file_iv(string(fout + ".iv").c_str(), ios::out | ios::binary);
    file_iv.write((char*)iv, GOST::BLOCKSIZE);
    file_iv.close();
    cout << "IV recorded in: " << fout << ".iv" << endl;
...
bool modGOST::decrypt()
{
    ...
    byte iv[GOST::BLOCKSIZE];
    ifstream file_iv(string(fin + ".iv").c_str(), ios::in | ios::binary);
    ...
    try {
        ...
        FileSource fs(fin.c_str(), true,
                      new StreamTransformationFilter(decr,
                              new FileSink(fout.c_str())));
    } catch (const Exception& e) {
        cerr << e.what() << endl;
        return false;
    }
